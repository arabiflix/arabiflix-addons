...
#
# Tuto ici :
# https://github.com/Kodi-netflix/venom-xbmc-addons/wiki
#
####################
...
class main:
    ...
    def parseUrl(self):        
        # Exclue les appels par des plugins qu'on ne sait pas g�rer, par exemple :  plugin://plugin.video.netflix/extrafanart
        oPluginHandler = cPluginHandler()
        pluginPath = oPluginHandler.getPluginPath()
        if pluginPath == 'plugin://plugin.video.netflix/extrafanart/':
            return
...
# Permet la modification des settings depuis un raccourci dans le skin (jusqu'� 100 param�tres).
# Supporte les retours � la ligne seulement derri�re le param�tre, exemple :
# RunAddon(plugin.video.netflix,function=setSettings&id1=plugin_cinemay_com&value1=true
...
def searchGlobal():
    ...
    for plugin in aPlugins:
        ...
        oGui.searchResults[:] = []  # vider le tableau de r�sultats pour les r�cup�rer par source
        _pluginSearch(plugin, sSearchText)

        if len(oGui.searchResults) > 0:  # Au moins un r�sultat
            count += 1
    ...
    if not count:   # aucune source ne retourne de r�sultats

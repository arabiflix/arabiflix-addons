...
the python script "\plugin.video.netflix\default.py" has left several classes in memory that we couldn't clean up. The classes include: class XBMCAddon::xbmcaddon::Addon

# https://stackoverflow.com/questions/26588266/xbmc-addon-memory-leak
"""
ADDONVS = xbmcaddon.Addon('plugin.video.netflix')  # singleton
...
class dialog:
    ...
    def VSok(self, desc, title='netflix'):
        return self.DIALOG.ok(title, desc)

    def VSyesno(self, desc, title='netflix'):
        return self.DIALOG.yesno(title, desc)

    def VSselect(self, desc, title='netflix'):
    ...
    def VSinfo(self, desc, title='netflix', iseconds=0, sound=False):
        if (iseconds == 0):
            iseconds = 1000
        else:
    ...
    def VSerror(self, e):
        return self.DIALOG.notification('netflix', 'Error: ' + str(e), xbmcgui.NOTIFICATION_ERROR, 2000), VSlog('Error: ' + str(e))

    def VStextView(self, desc, title='netflix'):
...
class progress:
    ...
    def VScreate(self, title='', desc='', large=False):
        ...
        # Ne pas afficher le sablier si nous ne sommes pas dans un menu netflix
...
class listitem(xbmcgui.ListItem):
    ...
    def addMenu(self, sFile, sFunction, sTitle, oOutputParameterHandler=False):
        sPluginPath = 'plugin://plugin.video.netflix/'  # cPluginHandler().getPluginPath()
...
def VSlog(e, level=xbmc.LOGDEBUG):
    try:
        ...
        xbmc.log('\t[PLUGIN] netflix: ' + str(e), level)
...
class siteManager:
    ...
    def __init__(self):
        ...
        self.defaultPath = VSPath('special://home/addons/plugin.video.netflix/resources/sites.json')
        self.defaultData = None

        # Propriétés selon le profil        
        name = VSProfil()
        if name == 'Master user':   # Le cas par defaut
            path = VSPath('special://home/userdata/addon_data/plugin.video.netflix/sites.json')
        else:
            path = VSPath('special://home/userdata/profiles/' + name + '/addon_data/plugin.video.netflix/sites.json')
